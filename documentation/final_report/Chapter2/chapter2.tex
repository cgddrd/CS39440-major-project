...
This experiment focussed on the extraction and subsequent matching of multiple overlapping small (no larger than 100px) square patches in order to provide an appearance-based approach to performing dense optical flow, in the hope that it would be possible to generate a vertical displacement model confirming the predicted behaviour stated in Section \ref{hypo-model}. 
...
\item Extract a percentage-width region of interest (ROI) from centre of first image.
\item Extract square patches of a fixed size around each pixel within the extracted ROI.
...
\item Average all measured vertical displacement values for each pixel along a given row, and append to vertical displacement model.
\end{enumerate}
