...
In the interests of time, the project decided to focus on the evaluation of four of the most popular similarity measures for template matching identified through the background research. These consisted of two `distance-based' measures, and two `histogram-based' measures.
...
In evaluating the performance of experiments, it was not simply the case that results would equate to a clearcut pass or fail. Instead, testing was much more of a ``subjective" task, involving the use of previously established ground truth data that an experiment would subsequently be expected to try and replicate as closely as possible. The performance of each specific method tested within an individual experiment would be assessed by comparing how closely each of the recorded results matched the results predicted. 
