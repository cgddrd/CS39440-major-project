...
Another approach proposed by T. Low and G. Wyeth \cite{low-wyeth} involves the combination of feature detection and appearance-based matching strategies in order to track corresponding features between video frames in order to estimate the optical flow field of a given scene. Firstly, initial features are detected using the same Shi and Tomasi corner detector \cite{shi-tomasi-good-features-to-track} as used in the approach devised by Campbell \textit{et al} \cite{campbell}. 
...
A key challenge relating to the use of optical flow for obstacle detection, and one discussed in the work of both Low and Wyeth, and Campbell \textit{et al} is the effect of disturbances in rotation (deliberate or otherwise)  on the observed optical flow of features. While both papers propose alternative methods for removing the effects of rotation (the use of an Inertial Measurement Unit gyroscope in the case of Low and Wyeth, and a calibrated cylindrical coordinate model in the case of Campbell \textit{et al}), both appear to agree that rotational movement should be measured separately to any translational movement in order to ensure accurate results can be obtained from the optical flow field. 

I. Ulrich and I. Nourbakhsh \cite{ulrich-nourbakhsh} present an altogether different approach to vision-based obstacle detection than those proposed by previously read papers. In their approach the authors focus on comparing differences in colour between the ground and other objects in the robot's field of view as a means of identifying potential obstacles. The authors argue that using colour as a detection metric can prove to be less computationally expensive than the ``range-based" obstacle detection systems investigated previously, and in certain cases, can prove to be more accurate and reliable - particularly when trying to detect small or flat objects close the ground or in situations where the system needs to be able to detect different types of terrain surface. 
