...
While these will of course only be estimates, it is hoped that when they are brought together, these observations could provide a vital insight into the overall status and condition of the current terrain and robot egomotion. 

The four ``condition metrics" subject to observation proposed under the hypothesis were as follows:
...
The general theory behind motion parallax is very simple, providing a visual cue from which the depth of objects situated within a 3-dimensional scene can subsequently be perceived. The concept describes how as the visual field of an observer changes due their movement, objects located at a short distance away will appear to move across the scene faster than those objects located further away.
 
 By measuring the level of displacement that an object demonstrates, it therefore becomes possible to infer the relative distance to that object from the current view position. Typical approaches within computer vision accomplish this task through the use of feature detection and tracking to calculate the displacement of static objects between consecutive images. 
 
One of the key advantages to using motion parallax within the context of computer vision and robotics comes from the fact that its effects continue to be visible even if captured via the use of only a single camera \cite{}. Therefore, strictly speaking, motion parallax can be best defined as providing a \textit{monocular} visual cue for depth, open to exploitation by any visual-based system defined as such through the use of at least one camera for providing input.
...
This process of measuring the displacement of pixels within select regions of an image is referred to as \textit{sparse optical flow} \cite{sparse-optical-flow} within the field of computer vision. By monitoring discontinuities in the length of these calculated optical flow vectors, Campbell \textit{et al.} are able to detect the presence of both positive and negative obstacles characterised as such by whether respectively, the pixels representing them hold vectors that are proportionally shorter or longer than the general majority \cite{campbell}.

The approach proposed within the hypothesis for this project aims to take the same fundamental idea, and apply it further to the inference of terrain gradient \textit{in addition to} the presence of potential obstacles. It is considered that, just in the same way as positive obstacles appear closer to the camera than the ground on which they lie, an area of ground that demonstrates a sudden increase in its tendency upwards would also appear closer to the camera, therefore holding optical flow vectors that are proportionally longer than those previously recorded within the same area of the previous image relative to the ground plane. Likewise, if presented with an area of ground that was to tend evenly downward, it would be expected that a significant decrease in the displacement would be detected as a result of observing shorter optical flow vectors.  

%The use of observed changes in motion between images to estimate the shape and location of 3D-structures is a well-explored concept that has given rise to a plethora of innovative vision-based projects \cite{tiddeman}, \cite{snavely}.
